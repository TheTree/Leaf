...
class Cron_task extends IBOT_Controller {
    ...
    function mysql_to_mongo() {
        ...
        // check our previous and max
        $_previous = $this->cache->get('mongo_lastid');
        $_max = $this->cache->get('mongo_maxid');
        ...
        if (intval($_max) == 0 || intval($_previous) == 0) {
            $_max = $this->stat_m->get_max_id();
            $this->cache->write($_max, 'mongo_maxid');
            $_previous = 0;
        }
        ...
        // Lets load eh 10 people who are expired
        $results = $this->stat_m->transfer_mongo($_previous, intval(10));
        ...
        if (is_array($results) && count($results) > 0) {
            ...
            foreach ($results as $result) {
                ...
                if ($this->stat_m->account_exists($result['HashedGamertag']) == FALSE) {

                    // add it
                    $new_record = $this->h4_lib->get_profile($result['Gamertag'], FALSE, TRUE, $result['SeoGamertag']);

                    if ($new_record != FALSE) {
                        $this->stat_m->update_account($result['HashedGamertag'], array(
                            H4::STATUS  => $result['status']
                        ));

                        print "Updated : " . $result['Gamertag'] . " into MongoDB \n";
                    }
                } else {
                    print $result['Gamertag'] . " already exists in MongoDB :( \n";
                }
            }

            // store new data
            $this->cache->write($result['id'], 'mongo_lastid');
            print "Wrote: " . $result['id'] . " to cache. \n";
        } else {
            $this->cache->write($_previous + 1, 'mongo_lastid');
            print "Wrote: " . $_previous + 1 . " to cache. \n";
            return $this->update_gamertags();
        }
    }
}
