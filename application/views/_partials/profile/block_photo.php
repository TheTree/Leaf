<? if (($data['Expiration'] - THREEDAYS_IN_SECONDS - time() + FIVEMIN_IN_SECONDS) > 0): ?>
<script type="text/javascript">
    function show_recache() {
        $('#recache_button').show();
    }
    $(document).ready(function() {
        setTimeout(show_recache, <?= (($data['Expiration'] - THREEDAYS_IN_SECONDS - time() + FIVEMIN_IN_SECONDS) * 1000) ?>);
    });
</script>
<? endif; ?>
...
<? if ($data['InactiveCounter'] < INACTIVE_COUNTER): ?>
    <? if ((($data['Expiration'] - THREEDAYS_IN_SECONDS + FIVEMIN_IN_SECONDS) < time()) && $data['InactiveCounter'] < INACTIVE_COUNTER): ?>
        <a href="<?= base_url('gt/' . $data['SeoGamertag'] . "/recache"); ?>" class="btn btn-success pad10">Refresh</a>
    <? elseif (($data['Expiration'] - THREEDAYS_IN_SECONDS - time() + FIVEMIN_IN_SECONDS) > 0): ?>
